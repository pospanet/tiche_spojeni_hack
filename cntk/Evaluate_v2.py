...
map_file_path = os.path.abspath(os.path.abspath(cfg["CNTK"].MAP_DIRECTORY))
...
model_path = os.path.abspath(os.path.join(cfg["CNTK"].MODEL_DIRECTORY, cfg["CNTK"].MODEL_NAME))
...
BaseDir = "CarRegistrationPlateDetector"
userDirectory = UserDirectory(BaseDir)
imgDir = userDirectory.getImageDir("toDetect")

imgFilenames = getFilesInDirectoryByType(imgDir, ["jpg","png"])

for imgFilenameIndex,imgFilename in enumerate(imgFilenames):
    imagePath = os.path.join(imgDir, imgFilename)
    print("{}: {}".format(imgFilenameIndex, imgFilename))
    evalResult = evaluateimage(imagePath, "returntags")
    print(evalResult)